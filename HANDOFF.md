...
| `RUN_BUDGET_SEC: 840` | job 타임아웃(20분)에 걸려 통째로 취소되기 전에 스스로 끊는다. 남은 건 다음 실행으로. 이 값이 한 실행의 요약 상한(840→100건)을 정한다 |
| `concurrency` 그룹 | 동시 실행 시 DB 경합 방지 |

> 실제로 첫 실행이 20분 타임아웃에 걸려 취소됐고, 발행 이력이 저장되지 않아
> 재발행 위험이 발생했다. 위 3가지는 그 사고의 대응책이다.

**실행 빈도 — 설정대로 안 돈다 (2026-08-27 실측):**

cron 은 10분 주기인데 실제 실행은 **하루 6~32회, 간격 중앙값 42분(최대 11시간)** 이다.
GitHub 무료 티어의 스케줄 실행은 best-effort 라 보장되지 않는다. 대응:

- cron 을 `7,17,27,...` 로 **오프셋** — 예약이 몰리는 정각·10분 단위 슬롯을 피한다
- `RUN_BUDGET_SEC` 을 840 으로 올려 **한 번에 100건**까지 처리 — 실행이 드문 날
  (6회면 6×72=432건) Gemini 일일 한도 560건을 남기고 끝나던 문제를 없앤다
- 급할 땐 Actions 탭 → **Run workflow** 로 수동 실행

> **진짜 상한은 Gemini 일일 한도 560건이다.** 12시간 창에 새 기사가 300건 이상
> 들어오므로, 실행 빈도를 아무리 올려도 하루 560건 요약을 넘길 수 없다.
