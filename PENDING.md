...
## 7. 손으로 옮긴 글 1건 — 발행 이력 DB 가 옛 위치를 가리킨다

2026-09-16, 국내정책(🇰🇷한국정책) 탭에 잘못 실린 미 하원 법안 기사를 US Policy
탭으로 옮겼다(옛 msg 8731 삭제 → 새 msg 8776, 첫 해시태그도 `#미국정책` 으로).

**DB 는 일부러 건드리지 않았다.** 봇이 5시간 30분 루프로 `botstate.sqlite3` 을
계속 커밋하는 중이라, 지금 같은 파일을 밀어 넣으면 `save_state` 의 rebase 가
충돌해 그 회차 발행 이력이 통째로 날아간다(6번 항목과 같은 사고).

그래서 `published` 의 이 행은 아직 옛 값이다:

```
key   681977d557b0f935941360c956edb1a22774307ebd3e2894803e7a45f3a266c5
지금  message_id=8731 · thread_id=3 · category=국내정책   ← 8731 은 이미 지워졌다
맞는값 message_id=8776 · thread_id=59 · category=US Policy
```

**영향은 작다.** 중복 발행은 `seen`·`seen_urls` 가 막으므로 다시 나가지 않는다.
다만 `--resort`·`--purge` 가 이 행을 만나면 없는 메시지를 고치려다 실패한다.

**할 일:** 다음에 DB 를 만질 일이 생기면(루프가 쉬는 사이) 위 값으로 고친다.
`store.update_published_location(key, 8776, 59, "US Policy")` 한 줄이다.

**함께 남은 것:** 같은 시간대 다이제스트("🇰🇷한국정책 · 09월 17일 05시~06시 정리")
에는 그 기사 요약이 그대로 남아 있다. 다이제스트는 별도 메시지라 같이 안 옮겨진다.
사용자 결정으로 그대로 둔다.
